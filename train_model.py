...
def print_metrics(current_epoch, current_batch, total_batches, loss, edit_distance, start_time):
    ...
    for epoch in range(max_epochs):
        ...
        if not exit_early:
            ...
            print_metrics(current_epoch=epoch, current_batch=n_validation_batches-1, total_batches=n_validation_batches,
                          loss=loss, edit_distance=edit_distance, start_time=batch_time)
            print('#############################################################################################')
...
def evaluate_on_testing_set(model_in, testing_generator_in):
    ...
    print('Testing Accuracy of the model on the ', total,
          ' testing images. Loss: {} %'.format(testing_loss))
...
if __name__ == "__main__":
    ...
    model = Net() # ConvNet()
    model.share_memory()
    partition = ImportData.load_data_set_indexes(dataset_hdf5_path)
    ...
    #    logger.update_scalar('Loss/test', numpy.random.random())
    #    logger.update_scalar('Accuracy/train', numpy.random.random())
    #    logger.update_scalar('Accuracy/test', numpy.random.random())
