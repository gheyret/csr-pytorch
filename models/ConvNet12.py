...
class ConvNet12(nn.Module):
    ...
    def __init__(self):
        ...
        self.layer1x5 = nn.Sequential(
            nn.Conv2d(1, 21, kernel_size=[3, 3], stride=(2, 2), padding=[1, 2], dilation=2))  # 5x5

        self.layer1x7 = nn.Sequential(
            nn.Conv2d(1, 21, kernel_size=[3, 3], stride=(2, 2), padding=[2, 3], dilation=3))  # 7x7

        self.layer1 = nn.Sequential(
            nn.BatchNorm2d(64),
            nn.ReLU())

        self.layer2x3 = nn.Sequential(
            nn.Conv2d(64, 44, kernel_size=[3, 3], stride=(2, 2), padding=[0, 1], dilation=1))  # 3x3

        self.layer2x5 = nn.Sequential(
            nn.Conv2d(64, 42, kernel_size=[3, 3], stride=(2, 2), padding=[1, 2], dilation=2))  # 5x5

        self.layer2x7 = nn.Sequential(
            nn.Conv2d(64, 42, kernel_size=[3, 3], stride=(2, 2), padding=[2, 3], dilation=3))  # 7x7
        ...
        self.layer3 = nn.Sequential(
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.Conv2d(128, 256, kernel_size=[3, 3], stride=(2, 1), padding=[0, 1]))

        self.resBlock256 = ResBlock(256)

        self.layer4 = nn.Sequential(
            nn.BatchNorm2d(256),
            nn.ReLU(),
            nn.Conv2d(256, 512, kernel_size=[3, 3], stride=(2, 1), padding=[0, 1]))

        self.resBlock512 = ResBlock(512)
    ...
    def forward(self, x, input_lengths):
        ...
        out = self.layer1(out)
        out3x3 = self.layer2x3(out)
        out5x5 = self.layer2x5(out)
        out7x7 = self.layer2x7(out)
        out = torch.cat((out3x3, out5x5, out7x7), 1)
        out = self.layer2(out)
        out = self.resBlock128(out)
        out = self.layer3(out)
        out = self.resBlock256(out)
        out = self.layer4(out)
        out = self.resBlock512(out)
