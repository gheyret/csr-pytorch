...
class ConvNet15(nn.Module):
    """
    Same as ConvNet2 but using ReLU as activation function and 2x BLSTM with more hidden layers.
    """
    def __init__(self):
        super(ConvNet15, self).__init__()
        ...
        self.rnn = nn.Sequential(
            nn.LSTM(input_size=1536, hidden_size=512, num_layers=2, bidirectional=True, batch_first=False))
        self.batchNorm = SequenceWise(nn.BatchNorm1d(512))
        self.rnn2 = nn.Sequential(
            nn.LSTM(input_size=512, hidden_size=512, num_layers=2, bidirectional=True, batch_first=False))
