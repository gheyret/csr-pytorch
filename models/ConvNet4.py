...
class ConvNet4(nn.Module):
    """
    Same as ConvNet2 but using stride = 2 in time domain only once instead of 2 times.
    -> Slower evaluation of samples but slightly better results.
    Note: Striding 3 times leads to unstable training where loss fluctuates wildly.
    """
    def __init__(self):
        ...
        self.layer2 = nn.Sequential(
            nn.Conv2d(64, 128, kernel_size=[5, 5], stride=(2,  1), padding=[0, 2]),
            nn.BatchNorm2d(128),
            nn.Hardtanh(0, 20, inplace=True))
