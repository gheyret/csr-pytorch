...
class ConvNet14(nn.Module):
    """
    Same as ConvNet2 but using ReLU as activation function and 5x BLSTM.
    """
    def __init__(self):
        super(ConvNet14, self).__init__()
        ...
        self.batchNorm2 = SequenceWise(nn.BatchNorm1d(512))
        self.rnn3 = nn.Sequential(
            nn.LSTM(input_size=512, hidden_size=512, num_layers=1, bidirectional=True, batch_first=False))
        self.batchNorm3 = SequenceWise(nn.BatchNorm1d(512))
        self.rnn4 = nn.Sequential(
            nn.LSTM(input_size=512, hidden_size=512, num_layers=1, bidirectional=True, batch_first=False))
        self.batchNorm4 = SequenceWise(nn.BatchNorm1d(512))
        self.rnn5 = nn.Sequential(
            nn.LSTM(input_size=512, hidden_size=512, num_layers=1, bidirectional=True, batch_first=False))
    ...
    def forward(self, x, input_lengths):
        ...
        out, h = self.rnn5(out)
        out, _ = nn.utils.rnn.pad_packed_sequence(out)
        out = out.view(out.size(0), out.size(1), 2, -1).sum(2).view(out.size(0), out.size(1),
